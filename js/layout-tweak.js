...
function exportCss(data) {
  ...
  for (const id of Object.keys(data).sort((a, b) => Number(a) - Number(b))) {
    ...
    for (const part of Object.keys(cell)) {
      ...
      if (!x && !y && s === 1) continue;
      lines.push(
        `.cell[data-id="${id}"] ${PART_SEL[part]} { --tw-x: ${x}%; --tw-y: ${y}%; --tw-s: ${s}; }`,
      );
    }
  }
  return lines.join('\n');
...
function applyAll(data) {
  ...
  for (const id of Object.keys(data)) {
    ...
    for (const part of Object.keys(cell)) {
      ...
      chunks.push(
        `.cell[data-id="${id}"] ${PART_SEL[part]}{--tw-x:${p.x}%;--tw-y:${p.y}%;--tw-s:${p.s};}`,
      );
    }
  }
  ...
  for (const boxId of ['layout-sliders', 'layout-own-sliders']) {
    ...
    for (const axis of ['x', 'y', 's']) {
      ...
      label.innerHTML =
        ...
        `<input type="range" min="${isS ? '0.4' : '-50'}" max="${isS ? '2.5' : '50'}"` +
...
function onPointerMove(e) {
  if (!drag) return;
  const data = loadSaved();
  const cell = document.querySelector('.cell[data-id="' + drag.cellId + '"]');
  const rect = cell?.getBoundingClientRect();
  const rw = rect?.width || 1;
  const rh = rect?.height || 1;
  const x = Math.round((drag.baseX + ((e.clientX - drag.startX) / rw) * 100) * 10) / 10;
  const y = Math.round((drag.baseY + ((e.clientY - drag.startY) / rh) * 100) * 10) / 10;
