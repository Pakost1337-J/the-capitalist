...
const TOP_EULER = {
  // Which face points up (+Y) under top-down camera
  1: [-90 * DEG, 0, 0],
  2: [0, 0, 0],
  3: [0, 0, 90 * DEG],
  4: [0, 0, -90 * DEG],
...
const CAM_THROW = { x: 0, y: 5.8, z: 9.2 };
const CAM_TOP = { x: 0, y: 7.2, z: 0.02 };
...
function finalQuat(value, twistZ = 0) {
  ...
  // Put target face up, then twist around world +Y
...
export class DiceScene {
  constructor(container) {
    ...
    this.rest = {
      a: { x: -0.85, z: 0.1, twist: -6 * DEG },
      b: { x: 0.85, z: -0.08, twist: 10 * DEG },
    };
  ...
  async _moveCamera(from, to, ms) {
    await animateValue(ms, easeInOutCubic, (e) => {
  ...
  setValues(v1, v2, { doubles = false } = {}) {
    ...
    if (!this.throwing) {
      this._placeRest(this.values[0], this.values[1]);
      if (this._camMode !== 'top') {
        this._setCam(CAM_TOP);
        this._camMode = 'top';
      }
    }
  ...
  async throw(v1, v2, { doubles = false } = {}) {
    ...
    this._setCam(CAM_THROW);
    this._camMode = 'throw';

    await Promise.all([
      this._throwOne(this.groupA, this.dieA, values[0], this.rest.a, 0),
      this._throwOne(this.groupB, this.dieB, values[1], this.rest.b, 120),
    ]);

    await this._moveCamera(CAM_THROW, CAM_TOP, 700);
    this._camMode = 'top';
  ...
  _throwOne(group, die, value, rest, delayMs) {
    return new Promise((resolve) => {
      ...
      const spin = new THREE.Quaternion().setFromEuler(new THREE.Euler(
        (540 + Math.random() * 360) * DEG,
        (360 + Math.random() * 360) * DEG,
        (180 + Math.random() * 180) * DEG,
        'XYZ',
      ));
      const midQ = startQ.clone().multiply(spin);
      const peakY = 3.4 + Math.random() * 0.5;
      const startX = rest.x * 0.15;
      const startZ = -4.8 - Math.random() * 0.6;
      group.position.set(startX, 2.2, startZ);
      die.quaternion.copy(startQ);
      ...
      const step = (now) => {
        ...
        let q;
        if (t < 0.42) {
          const u = easeOutCubic(t / 0.42);
          y = 2.2 + (peakY - 2.2) * u;
          q = startQ.clone().slerp(midQ, u);
          group.position.x = startX + (rest.x - startX) * u * 0.35;
          group.position.z = startZ + (rest.z - startZ) * u * 0.35;
        } else if (t < 0.82) {
          const u = easeInOutCubic((t - 0.42) / 0.4);
          y = peakY + (SIZE / 2 - peakY) * u;
          q = midQ.clone().slerp(endQ, Math.min(1, u * 1.15));
          group.position.x = startX + (rest.x - startX) * (0.35 + 0.65 * u);
          group.position.z = startZ + (rest.z - startZ) * (0.35 + 0.65 * u);
        } else {
          const u = (t - 0.82) / 0.18;
          y = SIZE / 2 + Math.sin(u * Math.PI) * 0.28 * (1 - u);
          q = endQ;
